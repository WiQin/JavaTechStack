...
>案例：
>- 现有一个咖啡店订单系统，项目设计较为简单，一个*Beverage*抽象类，包含了description变量和cost()方法，不同类型的饮品通过继承该类来定义其属性和价格  
>- 由于饮品种类和配料奇多，导致完成的程序下有大量的类继承了Beverage，这个时候如果某种配料的价格发生改变或者新增了一种配料，对于项目的维护来说是一个巨大的灾难  
>- 所以，继承不是万能的，利用继承设计子类的行为，是在编译时静态决定的，而且所有的子类都会集成到相同的行为；我们要做的是通过程序设计尽量减少现有代码的修改同时扩展新的功能，以此避免程序修改带来的影响
...
>   - 我们的目标是允许类容易扩展，在不修改现有代码的情况下，就可搭配新的行为。这样的设计具有弹性可以应对改变

如何在程序设计中做到上述原则呢?装饰者模式就是个很好的例子
怎么解决呢，答案就是以饮品为主体，在运行时以调料来装饰它

>首先看下装饰者模式的定义：
>装饰者模式是以对客户端透明的方式扩展对象的功能，是继承关系的一个替代方案。装饰者模式动态地将责任附加到对象上，若要扩展功能，装饰者提供了比继承更有弹性的替代方案

>装饰着模式有哪些特点呢?
>1. 装饰者和被装饰对象有相同的超类型-通过继承达到”类型匹配“
>2. 可以用一个或者多个装饰者包装一个对象
>3. 由于装饰者和被装饰对象有相同的超类型，所以在任务需要原始对象的场合，都可以使用装饰过的对象代替
>4. 装饰者可以在所委托被装饰者的行为之前/后加上自己的行为，以达到某种特定的目的

看着是不是有点晕?那我们看图说话吧(摘自*HeadFirst设计模式*)
![decorator_uml.png](../static/image/java/design_pattern/decorator/decorator_uml.png)
...
> 一个命令对象通过在特定接收者上绑定一组对象来封装一个请求，怎么实现？
>命令对象将动作和接收者包进对象中，这个对象只暴露一个execute()方法，当此方法被调用时，接收者就会进行这些动作
>外层对象并不知道哪个接收者进行了哪些动作，只知道如果调用execute方法，请求的目的就能达到 
